## Author: User                                                       ##
## Date: 12/5/2022                                                        ##
## Purpose: Combine draws for ensemble model: stratified, unweighted ,visits peru, constant and no constant
################################################################################
## clear memory
rm(list=ls())
...
level_lag_no_constant <-fread(file.path(dir, 'per_visits_draws_lag_model_no_constant.csv'))

## diff draws, no constant
diff_draws_no_constant <-fread(file.path(dir, 'per_visits_draws_diff_model_no_constant.csv'))
...
write.csv(visits_draws_summary,file.path(dir, 'visits_plotting_predictions_no_constant.csv'), row.names = F)
