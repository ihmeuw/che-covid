## Author: User                                                       ##
## Purpose: Predict out of sample each year CHE using level ~ lag level, unweighted  (model 2), no constant            ##
################################################################################
## clear memory
rm(list=ls())
...
predict_holdouts <- function(loc) {
  ...
  for (year in unique(years$V1)){
    temp_lm_che10 <- lm(che10 ~ lag_che-1, data = data_che10[year_id %nin% c(`year`,2020)])
    temp_lm_che25 <- lm(che25 ~ lag_che-1, data = data_che25[year_id %nin% c(`year`,2020)])
...
#save CSV's 
write.csv(che10_compare,'FILEPATH/che10_compare_holdout_model2_unweighted_no_constant.csv', row.names = F)
write.csv(che25_compare,'FILEPATH/che25_compare_holdout_model2_unweighted_no_constant.csv', row.names = F)
