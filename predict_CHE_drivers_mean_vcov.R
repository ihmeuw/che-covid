## Author: User                                                       ##
## Date: 11/11/2022                                                        ##
## Purpose: Predict 2020 CHE based on mean values in previous year with vcov: value ~ lag value
################################################################################
## clear memory
rm(list=ls())
...
predict_var <- function (current_ihme_loc_id, current_var){
  ...
  m1 <- lm(value ~ lag_value, data = che_driver_means[year_id != 2020 & ihme_loc_id == current_ihme_loc_id & variable == current_var])
  ...
  check[, constant := 1]
  check <- check[, c("constant","lag_value", "draw", "year_id", "value")]
  ...
  temp[, value_predict := (Intercept * constant) + (coeff * lag_value)]
...
write.csv(figure_a_draws,file.path(dir, 'figure_a_draws_lag_model.csv'), row.names = F)

visits_draws <- do.call("rbind", list(per_visits[[2]], per_private[[2]], per_share_private[[2]]))
write.csv(visits_draws,file.path(dir, 'per_visits_draws_lag_model.csv'), row.names = F)
