#####################################################################
## Author: User                                           ##
## Description: Peru visit analysis table 1          ##
##                                                                 ##
#####################################################################
...
## libraries
pacman::p_load(data.table, openxlsx, haven, readstata13, readxl, ggplot2, survey, broom)
...
visits20 <- fread(file.path(indir, 'PER_ENAHO_VISITS_2020_updated.csv'))[,c("nconglome", "estrato", "factor07", "is_private", "year_id", "any_visit")]
visits19<- fread(file.path(indir, 'PER_ENAHO_VISITS_2019_updated.csv'))[,c("nconglome", "estrato", "factor07", "is_private", "year_id", "any_visit")]

visits <- rbind(visits19, visits20)
visits[, year_factor := as.factor(as.numeric(year_id)-2019)]
...
design <-  svydesign(ids = ~nconglome, strata = ~estrato, weights = ~factor07, data = visits, variables = ~share_private+year_factor, nest = T)
...
write.csv(long_pvalue, file.path(outdir, 'peru_table1_visits_private.csv'))
write.csv(long_result, file.path(outdir, 'peru_table1_visit_ins_ci_se.csv'))
