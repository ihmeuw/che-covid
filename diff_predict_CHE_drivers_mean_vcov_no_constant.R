## Author: User                                                       ##
## Date: 11/11/2022                                                        ##
## Purpose: Predict 2020 CHE based on mean diffs in previous year with vcov: diff ~ lag diff, no constant
################################################################################
## clear memory
rm(list=ls())
...
predict_var <- function (current_ihme_loc_id, current_var){
  ...
  m1 <-  lm(diff ~ lag_diff -1, data = che_driver_means[year_id != 2020 & ihme_loc_id == current_ihme_loc_id & variable == current_var])
  ...
  check <- check[, c("lag_diff", "draw", "year_id", "diff", "lag_value")]
  setnames(driver_model, old = "lag_che", new = "coeff")
  predictions <- data.table()
  
  temp <- cbind(check, driver_model)
  temp[, diff_predict := coeff * lag_diff]
...
write.csv(figure_a_draws,file.path(dir, 'figure_a_draws_diff_model_no_constant.csv'), row.names = F)

visits_draws <- do.call("rbind", list(per_visits[[2]], per_private[[2]], per_share_private[[2]]))
write.csv(visits_draws,file.path(dir, 'per_visits_draws_diff_model_no_constant.csv'), row.names = F)
