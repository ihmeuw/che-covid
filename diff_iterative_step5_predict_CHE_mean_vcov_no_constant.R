## Author: User                                                       ##
## Date: 10/10/2022                                                        ##
## Purpose: Predict 2020 CHE based on mean values in previous year
################################################################################
## clear memory
rm(list=ls())
...
## read in draws
che10_draws <- fread(file.path(dir, 'all_draws_che10_10000_rate_diff.csv'))
che25_draws <- fread(file.path(dir, 'all_draws_che25_10000_rate_diff.csv'))

# read in predicted 2019 values for russia 
che10_predict_2019_rus <- fread(file.path(dir, 'predict_2019_russia_che10_no_constant.csv'))
che25_predict_2019_rus <- fread(file.path(dir, 'predict_2019_russia_che25_no_constant.csv'))

## read in mean df's
che10_mean <- fread(file.path(dir, 'che10_mean_df_diff.csv'))
che25_mean <- fread(file.path(dir, 'che25_mean_df_diff.csv'))
...
predict_che10_rus <- function (current_ihme_loc_id){
  weight_num <- numpoints[ihme_loc_id == current_ihme_loc_id]$num_points
  che10_model <- data.table()
  m1 <- lm(che_diff ~ lag_diff -1 , data = che10_mean[year_id != 2020 & ihme_loc_id == current_ihme_loc_id],weights =  c(c(0.25, 0.2, 0.15,0.075, 0.075), rep(0.025,weight_num)))
  summary_m1 <- tidy(m1)
  nobs(m1)
  set.seed(123)
  n.sim <- 1000
  ...
  check <- check[, c("lag_diff", "draw", "year_id", "che_diff", "lag_che")]
  setnames(che10_model, old = "lag_che", new = "coeff")
  
  # fill in 2015 
  starting_predict <- data.table()
  starting_predict$lag_diff <- check[year_id == 2014]$che_diff
  ...
  temp[, che10_diff_predict := coeff * lag_diff]
  missing_predictions <- rbind(missing_predictions,temp[, c("year_id", "che10_diff_predict", "draw")])
  
  years <- c( 2016, 2017, 2018, 2019, 2020) # years to predict after last point of 2014
  for (year in years){ 
    current <- data.table()
    #check_t <- missing_predictions[year_id == 2015]
    current$lag_diff <- missing_predictions[year_id == year-1]$che10_diff_predict
  ...
  missing_predictions_2020 <- missing_predictions[year_id == 2020, c("che10_diff_predict")]
  missing_predictions_2020 <- cbind(missing_predictions_2020,che10_predict_2019_rus )
  missing_predictions_2020 <- missing_predictions_2020[, che10_predict := che10_predict +che10_diff_predict] 
  ...
  return_list  <- list(che10_predictions,missing_predictions_2020[, c("che10_predict")])
  
  return(return_list)
}


predict_che25_rus <- function (current_ihme_loc_id){
  weight_num <- numpoints[ihme_loc_id == current_ihme_loc_id]$num_points
  che25_model <- data.table()
  m1 <- lm(che_diff ~ lag_diff -1 , data = che25_mean[year_id != 2020 & ihme_loc_id == current_ihme_loc_id],weights =  c(c(0.25, 0.2, 0.15,0.075, 0.075), rep(0.025,weight_num)))
  summary_m1 <- tidy(m1)
  nobs(m1)
  set.seed(123)
  n.sim <- 1000
  ...
  temp[, che25_diff_predict := coeff * lag_diff]
  missing_predictions <- rbind(missing_predictions,temp[, c("year_id", "che25_diff_predict", "draw")])
  
  years <- c( 2016, 2017, 2018, 2019, 2020) # years to predict after last point of 2014
  for (year in years){ 
    current <- data.table()
    current$lag_diff <- missing_predictions[year_id == year-1]$che25_diff_predict
    current$draw <- missing_predictions[year_id == year-1]$draw
    current$year_id <- year
  ...
  missing_predictions_2020 <- missing_predictions[year_id == 2020, c("che25_diff_predict")]
  missing_predictions_2020 <- cbind(missing_predictions_2020,che25_predict_2019_rus )
  missing_predictions_2020 <- missing_predictions_2020[, che25_predict := che25_predict +che25_diff_predict] 
  ...
  return_list  <- list(che25_predictions,missing_predictions_2020[, c("che25_predict")])
  
  return(return_list)
}
current_ihme_loc_id <- "VNM"
predict_che10 <- function (current_ihme_loc_id){
  weight_num <- numpoints[ihme_loc_id == current_ihme_loc_id]$num_points
  che10_model <- data.table()
  m1 <- lm(che_diff ~ lag_diff -1, data = che10_mean[year_id != 2020  & ihme_loc_id == current_ihme_loc_id])
...
nobs(m1)
  ...
  temp[, che10_predict := lag_che + (coeff * lag_diff)]
...
predict_che25 <- function (current_ihme_loc_id){
  weight_num <- numpoints[ihme_loc_id == current_ihme_loc_id]$num_points
  che25_model <- data.table()
  m1 <- lm(che_diff ~ lag_diff -1, data = che25_mean[year_id != 2020 & ihme_loc_id == current_ihme_loc_id])
  ...
  temp[, che25_predict := lag_che + (coeff * lag_diff)]
...
rus_che10_predict <- predict_che10_rus("RUS")[[1]]
...
rus_che25_predict <- predict_che25_rus("RUS")[[1]]
...
# save predictions based off the mean
write.csv(che10_predictions,file.path(dir, 'che10_predict_10000_adjust_mean_diff_vcov_iter_rus_no_constant.csv'), row.names = F)
write.csv(che25_predictions,file.path(dir, 'che25_predict_10000_adjust_mean_diff_vcov_iter_rus_no_constant.csv'), row.names = F)
...
rus_che10_predict_draws <- predict_che10_rus("RUS")[[2]]  
...
rus_che25_predict_draws <- predict_che25_rus("RUS")[[2]]  
...
write.csv(che10_predict_draws,file.path(dir, 'che10_predict_draws_diff_no_constant.csv'), row.names = F)
write.csv(che25_predict_draws,file.path(dir, 'che25_predict_draws_diff_no_constant.csv'), row.names = F)
