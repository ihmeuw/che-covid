## Author: User                                                       ##
## Purpose: Predict 2019 CHE using diff ~ lag diff, unweighted               ##
################################################################################
## clear memory
rm(list=ls())
...
belarus_che10 <- fread('FILEPATH/data/hefpi_21.csv')[location_id==57]
...
#Peru 
per_5_che10 <- lm(che_diff ~ lag_diff, data = che10_df[ihme_loc_id == "PER" & year_id %nin% c(2020)])
per_5_che25 <-lm(che_diff ~ lag_diff, data = che25_df[ihme_loc_id == "PER" & year_id %nin% c(2020)])
...
#Mexico
mex_5_che10 <- lm(che_diff ~ lag_diff, data = che10_df[ihme_loc_id == "MEX"& year_id %nin% c(2020)])
mex_5_che25 <- lm(che_diff ~ lag_diff, data = che25_df[ihme_loc_id == "MEX"& year_id %nin% c(2020)])
...
#Belarus
bel_5_che10 <- lm(che_diff ~ lag_diff, data = che10_df[ihme_loc_id == "BLR"& year_id %nin% c(2020)])
bel_5_che25 <-  lm(che_diff ~ lag_diff, data = che25_df[ihme_loc_id == "BLR"& year_id %nin% c(2020)])
...
#Russia
rus_5_che10 <- lm(che_diff ~ lag_diff, data = che10_df[ihme_loc_id == "RUS"& year_id %nin% c(2020)])
rus_5_che25 <-  lm(che_diff ~ lag_diff, data = che25_df[ihme_loc_id == "RUS"& year_id %nin% c(2020)])
...
#Vietnam.
vnm_5_che10 <- lm(che_diff ~ lag_diff, data = che10_df[ihme_loc_id == "VNM"& year_id %nin% c(2020)])
vnm_5_che25 <-  lm(che_diff ~ lag_diff, data = che25_df[ihme_loc_id == "VNM"& year_id %nin% c(2020)])
...
che10_df_predict_5_years_per <- che10_df %>%
  ...
  mutate(che10_diff_predict = NA)
che25_df_predict_5_years_per <- che25_df %>%
  filter(ihme_loc_id == "PER") %>%
  mutate(che25_diff_predict =NA)
...
#Peru
#CHE 10
peru_che10_predict <- predict_uncertainty(che10_df_predict_5_years_per, per_5_che10, "che10_diff_predict")

#CHE 25
peru_che25_predict <- predict_uncertainty(che25_df_predict_5_years_per, per_5_che25, "che25_diff_predict")

#Mexico
#CHE 10
mexico_che10_predict <-  predict_uncertainty(che10_df_predict_5_years_mex, mex_5_che10, "che10_diff_predict")

#CHE 25
mexico_che25_predict <-  predict_uncertainty(che25_df_predict_5_years_mex, mex_5_che25, "che25_diff_predict")

#Belarus
#CHE 10
belarus_che10_predict <-predict_uncertainty(che10_df_predict_5_years_blr, bel_5_che10, "che10_diff_predict")

#CHE 25
belarus_che25_predict <-  predict_uncertainty(che25_df_predict_5_years_blr, bel_5_che25, "che25_diff_predict")

#Russia
#CHE 10
russia_che10_predict <- predict_uncertainty(che10_df_predict_5_years_rus, rus_5_che10, "che10_diff_predict")

#CHE 25
russia_che25_predict <-  predict_uncertainty(che25_df_predict_5_years_rus, rus_5_che25, "che25_diff_predict")

#Vietnam
#CHE 10
vietnam_che10_predict <- predict_uncertainty(che10_df_predict_5_years_vnm, vnm_5_che10, "che10_diff_predict")

#CHE 25
vietnam_che25_predict <- predict_uncertainty(che25_df_predict_5_years_vnm, vnm_5_che25, "che25_diff_predict")


#merge dfs 
## CHE 10
che10_predict <- rbind(peru_che10_predict[,c("year_id", "che10", "ihme_loc_id","che10_diff_predict", "lag_che")],
                       mexico_che10_predict[,c("year_id", "che10", "ihme_loc_id","che10_diff_predict", "lag_che")],
                       belarus_che10_predict[,c("year_id", "che10", "ihme_loc_id","che10_diff_predict", "lag_che")],
                       russia_che10_predict[,c("year_id", "che10", "ihme_loc_id","che10_diff_predict", "lag_che")],
                       vietnam_che10_predict[,c("year_id", "che10", "ihme_loc_id","che10_diff_predict", "lag_che")])

## CHE 25
che25_predict <- rbind(peru_che25_predict[,c("year_id", "che25", "ihme_loc_id","che25_diff_predict", "lag_che")],
                       mexico_che25_predict[,c("year_id", "che25", "ihme_loc_id","che25_diff_predict", "lag_che")],
                       belarus_che25_predict[,c("year_id", "che25", "ihme_loc_id","che25_diff_predict", "lag_che")],
                       russia_che25_predict[,c("year_id", "che25", "ihme_loc_id","che25_diff_predict", "lag_che")],
                       vietnam_che25_predict[,c("year_id", "che25", "ihme_loc_id","che25_diff_predict", "lag_che")])
...
#save CSV's 
write.csv(che10_predict[,-c("che10_diff_predict", "lag_che" )] ,'FILEPATH/che10_compare_model3_all_years_unweighted.csv', row.names = F)

write.csv(che25_predict[,-c("che25_diff_predict", "lag_che" )],'FILEPATH/che25_compare_model3_all_years_unweighted.csv', row.names = F)
