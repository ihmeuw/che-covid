## Author: User                                                    ##
## Purpose: Predict 2019 CHE using  diff ~ lag diff + lag diff (2), unweighted               ##
################################################################################
## clear memory
rm(list=ls())
...
# Model 5: diff ~ lag diff + lag diff (2) (unweighted)
...
#Peru 
per_5_che10 <- lm(che_diff ~ lag_diff + lag_diff_2, data = che10_df[ihme_loc_id == "PER" & year_id %nin% c(2019,2020)])
per_5_che25 <-lm(che_diff ~ lag_diff + lag_diff_2, data = che25_df[ihme_loc_id == "PER" & year_id %nin% c(2019,2020)])

#Mexico
mex_5_che10 <- lm(che_diff ~ lag_diff + lag_diff_2, data = che10_df[ihme_loc_id == "MEX"& year_id %nin% c(2018,2020)])
mex_5_che25 <- lm(che_diff ~ lag_diff + lag_diff_2, data = che25_df[ihme_loc_id == "MEX"& year_id %nin% c(2018,2020)])

#Belarus
bel_5_che10 <- lm(che_diff ~ lag_diff + lag_diff_2, data = che10_df[ihme_loc_id == "BLR"& year_id %nin% c(2019,2020)])
bel_5_che25 <-  lm(che_diff ~ lag_diff + lag_diff_2, data = che25_df[ihme_loc_id == "BLR"& year_id %nin% c(2019,2020)])

#Russia
rus_5_che10 <- lm(che_diff ~ lag_diff + lag_diff_2, data = che10_df[ihme_loc_id == "RUS"& year_id %nin% c(2014,2020)])
rus_5_che25 <-  lm(che_diff ~ lag_diff + lag_diff_2, data = che25_df[ihme_loc_id == "RUS"& year_id %nin% c(2014,2020)])

#Vietnam.
vnm_5_che10 <- lm(che_diff ~ lag_diff + lag_diff_2, data = che10_df[ihme_loc_id == "VNM"& year_id %nin% c(2018,2020)])
vnm_5_che25 <-  lm(che_diff ~ lag_diff + lag_diff_2, data = che25_df[ihme_loc_id == "VNM"& year_id %nin% c(2018,2020)])
...
#save CSV's 
write.csv(che10_compare,'FILEPATH/che10_compare_model5_unweighted.csv', row.names = F)

write.csv(che25_compare,'FILEPATH/che25_compare_model5_unweighted.csv', row.names = F)
